...
class onBeat {
  ...
  promiseStep = async(mark, cb) => new Promise((resolve, reject) => {
    let id;
    ...
    // queing with a the mark, so that it can be resolved without using multiple loops
    this.promiseQue[mark] = this.promiseQue[mark] 
      ? [].concat(this.promiseQue[mark], (data) => resolve(cb(data)))
      : [ (data) => resolve(cb(data)) ]
    ...
    let step = async (timestamp) => {
      ...
      if ( this.beatMark === mark || this.checkCustomMark(this.beatMark, mark) ) {
        ...
        // when the que is empty there is no need to loop. There are no callbacks.
        if ( !(<any>Object).getOwnPropertySymbols(this.promiseQue).length ) {
          this.looping = false
          window.cancelAnimationFrame(id)
        }
        
        try {
          let args = { id, timestamp, mark }
          ...
          this.promiseQue[mark].forEach( resolve => {
            resolve(args)
          })
          delete this.promiseQue[mark]
        } catch (error) {
          console.warn("callback threw error", error.message)
          // maybe shouldn't lose the callback reference
          delete this.que[markSymbol]
          reject()
        }
        // this.looping = false
